...
class UnsafeLaneChangeOracle(OracleInterface):
    ...
    def on_new_message(self, topic: str, message, t):
        ...
        self.searchable_boundary_ids = (
            self.searchable_boundary_ids - pending_removal_boundary_ids
        )
        # no intersection
        self.__data.append((False, t, "", {}))
    ...
    def get_result(self):
        ...
        for k, v in groupby(self.__data, key=lambda x: (x[0], x[2])):
            ...
            # print(self.boundary_ids.index(38))
            traces = list(v)
            if intersects and len(traces) > 1:
                ...
                if (
                    delta_t
                    > self.ADC_INTERSECTING_LANE_BOUNDARY_MAX_LOOK_BACK_FRAMES_IN_SECOND
                ):
